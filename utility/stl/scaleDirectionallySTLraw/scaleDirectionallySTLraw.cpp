...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        pcout << "Wrong parameters; the syntax is: " << (std::string)global::argv(0)
              << " inputSTL.stl xScale yScale zScale outputSTL.stl" << std::endl;
        pcout << "If for example, you want your STL to be rescaled by a factor 0.01 in x and 0.02 "
                 "in z, you should write"
              << std::endl;
        pcout << (std::string)global::argv(0) << " inputSTL.stl 0.01 1.0 0.02 outputSTL.stl"
              << std::endl;
        exit(-1);
    }
