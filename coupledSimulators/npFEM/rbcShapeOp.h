...
template <typename T>
inline void controlPositionPeriodic(
    Array<T, 3> &pos, const pluint lx, const pluint ly, const pluint lz, bool &corrected_x,
    bool &corrected_y, bool &corrected_z)
{
...
public:
    ...
    void init(
        pluint bodyID_, RawConnectedTriangleMesh<T> *templateMesh_,
        std::vector<int> *mesh_graph = NULL)
    {
    ...
    ShapeOpBody(
        pluint bodyID_, ShapeOp_Solver &solver_, RawConnectedTriangleMesh<T> *templateMesh_) :
        bodyID(bodyID_),
        solver(solver_),
        templateMesh(templateMesh_),
        numVertices_onSurface_(numVertices_onSurface()),
    ...
    void receiveContainers(
        // Force Containers
        std::vector<pluint> const &vertexIDs, std::vector<Array<T, 3>> const &plbShearForces,
        std::vector<Array<T, 3>> const &plbNormals, std::vector<T> const &plbPressure,
        std::vector<T> const &plbArea, pluint taskID,
        ...
        std::vector<Array<T, 3>> newNeighbors, std::vector<Array<T, 3>> newNeighborsNormals)
    {
        ...
        for (pluint i = 0; i < (pluint)vertexIDs.size(); ++i) {
            ...
            SOLocalMeshCenterOfMass += Array<T, 3>(
                points.col(vertexIDs[i])[0], points.col(vertexIDs[i])[1],
        ...
        if (newNeighbors.size() > 0) {
            ...
            collisionNeighbors.insert(
                collisionNeighbors.end(), newNeighbors.begin(), newNeighbors.end());
            collisionNeighborsNormals.insert(
                collisionNeighborsNormals.end(), newNeighborsNormals.begin(),
                newNeighborsNormals.end());
        }
    }
    ...
    void applyForces(bool CellPacking)
    {
        ...
        for (pluint i = 0; i < numVertices(); ++i) {
            if (onSurfaceParticle[i]) {
                ...
                commForces.col(i) =
                    shearForces.col(i) + (pressure[i] - p0) * area[i] * normals.col(i).normalized();
            }
        }
    ...
    void applyCollidingNeighbors()
    {
        ...
        for (pluint i = 0; i < (pluint)neighbors.cols(); ++i) {
            neighbors.col(i) = plb::npfem::Vector3(
                collisionNeighbors[i][0], collisionNeighbors[i][1], collisionNeighbors[i][2]);

            neighborsNormals.col(i) = plb::npfem::Vector3(
                collisionNeighborsNormals[i][0], collisionNeighborsNormals[i][1],
    ...
    void sendVelocities()
    {
        ...
        for (pluint i = 0; i < numTasksToSendVelocities(); ++i) {
            ...
            if (newVelocities.size() > 0 && taskID != (pluint)global::mpi().getRank()) {
                ...
                MPI_Isend(
                    &velocities.back()[0][0], (int)(3 * velocities.back().size()), MPI_DOUBLE,
                    (int)taskID, shapeOpMPItagVelocities, MPI_COMM_WORLD, mpiRequests.back());
            }
        }
    }
...
#ifdef ENABLE_LOGS
        plb::global::logfile_nonparallel("localProgress.log")
            .flushEntry("bodyID=" + util::val2str(bodyID) + ", Start MPI Completion (Velocities)");
#endif  // ENABLE_LOGS
...
#ifdef ENABLE_LOGS
        plb::global::logfile_nonparallel("localProgress.log")
            .flushEntry("bodyID=" + util::val2str(bodyID) + ", Finish MPI Completion (Velocities)");
#endif  // ENABLE_LOGS
    }
    ...
    const int nb_cells, double dx) { typedef Array<Array<T, 3>, 3> RawTriangle;
        ...
        for (int c = 0; c < nb_cells; c++) {
            for (const auto &i : triangles) {
                plb_triangles[k] = Array<plint, 3>(i[0] + c*npoint, i[1] + c*npoint, i[2] +
    c*npoint);
                //std::cout << triangles[k][0]<< " " << triangles[k][1]<< " " << triangles[k][2] <<"
    |" << k <<"\n"; k++;
            }
        }
    ...
    */
    ...
    RawConnectedTriangleMesh<T> mergeMultipleMeshes(
        std::vector<plb::npfem::Solver *> &rbc, std::vector<plb::npfem::Solver *> &plt, T dx,
        T dump_RBCs_ratio = 1.0, T dump_PLTs_ratio = 1.0)
    {
        ...
        // Connectivity of the Surface only
        for (int j = 0; j < (int)((T)rbc.size() * dump_RBCs_ratio); j++) {
            for (auto i = rbc[j]->getConnectivityList().begin();
                 i < rbc[j]->getConnectivityList().end(); ++i)
                triangles.push_back(
                    Array<plint, 3>((*i)[0] + last_p, (*i)[1] + last_p, (*i)[2] + last_p));

            last_p += rbc[j]->getPoints().cols();
        }

        for (int j = 0; j < (int)((T)plt.size() * dump_PLTs_ratio); j++) {
            for (auto i = plt[j]->getConnectivityList().begin();
                 i < plt[j]->getConnectivityList().end(); ++i)
                triangles.push_back(
                    Array<plint, 3>((*i)[0] + last_p, (*i)[1] + last_p, (*i)[2] + last_p));
