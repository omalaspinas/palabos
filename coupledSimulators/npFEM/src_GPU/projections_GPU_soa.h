...
namespace npfem {
////////////////////////////////////////////////////////////////////////////////
__device__ void project_volume_d(
    int n_points, int n_tri, int n_constraints, double *points_d, short *triangles_d,
    cuda_scalar *force, double *E_nonePD, cuda_scalar volume0, const int nb_sum_thread,
    double *buffer_double, cuda_scalar *grad_c, const int id, const float volume_weight)
{
    ...
    for (int tid = threadIdx.x; tid < n_tri; tid += blockDim.x) {
        ...
        buffer_double[threadIdx.x] +=
            area * (n0 * (p0x + p1x + p2x) + n1 * (p0y + p1y + p2y) + n2 * (p0z + p1z + p2z)) / 9;
    ...
    E_nonePD[blockIdx.x * n_constraints + threadIdx.x] +=
        0.5 * volume_weight * (dx * dx + dy * dy + dz * dz);

    // if(threadIdx.x == 224)printf("force volume %f %f %f | %d\n", force[id], force[id + n_points],
    // force[id + 2 * n_points], threadIdx.x);
}
///////////////////////////////////////////////////////////////////////////////
__device__ void project_Bending(
    int tid, int n_points, int n_constraints, int n_projected_points, int nb_cells,
    ShapeOpScalar *points_d, ShapeOpScalar *projections_d, cuda_scalar *f_int_nonePD_d,
    int *ConstraintType_d, int *idO_d, ShapeOpScalar *rangeMin_d, ShapeOpScalar *rangeMax_d,
    ShapeOpScalar *Scalar1_d, ShapeOpScalar *weight_d, ShapeOpScalar *E_nonePD_d, int *idI_d,
    ShapeOpScalar *vectorx_d, ShapeOpScalar *matrix22_d, ShapeOpScalar *matrix33_d)
{
    ...
    if (n_ > 1e-6) {
        e0 = w0_ * points_d[IDX(idI0_, 0, n_points)] + w1_ * points_d[IDX(idI1_, 0, n_points)]
             + w2_ * points_d[IDX(idI2_, 0, n_points)] + w3_ * points_d[IDX(idI3_, 0, n_points)];
        e1 = w0_ * points_d[IDX(idI0_, 1, n_points)] + w1_ * points_d[IDX(idI1_, 1, n_points)]
             + w2_ * points_d[IDX(idI2_, 1, n_points)] + w3_ * points_d[IDX(idI3_, 1, n_points)];
        e2 = w0_ * points_d[IDX(idI0_, 2, n_points)] + w1_ * points_d[IDX(idI1_, 2, n_points)]
             + w2_ * points_d[IDX(idI2_, 2, n_points)] + w3_ * points_d[IDX(idI3_, 2, n_points)];
    ...
    if (tid == 107){
        ...
        printf("e %f %f %f \n", e0, e1, e2);
        printf("projection %f %f %f\n", points_d[IDX(idI0_, 0, n_points)], points_d[IDX(idI0_, 1,
    n_points)], points_d[IDX(idI0_, 2, n_points)]); printf("projection %f %f %f\n",
    points_d[IDX(idI1_, 0, n_points)], points_d[IDX(idI1_, 1, n_points)], points_d[IDX(idI1_, 2,
    n_points)]); printf("projection %f %f %f\n", points_d[IDX(idI2_, 0, n_points)],
    points_d[IDX(idI2_, 1, n_points)], points_d[IDX(idI2_, 2, n_points)]); printf("projection %f %f
    %f\n", points_d[IDX(idI3_, 0, n_points)], points_d[IDX(idI3_, 1, n_points)], points_d[IDX(idI3_,
    2, n_points)]); printf("projection %f %f %f \n", projections_d[IDX(idO_, 0,
    n_projected_points)], projections_d[IDX(idO_, 1, n_projected_points)], projections_d[IDX(idO_,
    2, n_projected_points)]);
    }
    */
}
///////////////////////////////////////////////////////////////////////////////

__device__ void project_Area(
    int tid, int n_points, int n_constraints, int n_projected_points, int nb_cells,
    ShapeOpScalar *points_d, ShapeOpScalar *projections_d, cuda_scalar *f_int_nonePD_d,
    int *ConstraintType_d, int *idO_d, ShapeOpScalar *rangeMin_d, ShapeOpScalar *rangeMax_d,
    ShapeOpScalar *Scalar1_d, ShapeOpScalar *weight_d, ShapeOpScalar *E_nonePD_d, int *idI_d,
    ShapeOpScalar *vectorx_d, ShapeOpScalar *matrix22_d, ShapeOpScalar *matrix33_d)
{
    ...
    // if (tid == 768)printf("matrix2 %f %f %f %f tid %d\n", rest00_, rest10_, rest01_, rest11_,
    // tid);

    cuda_scalar edges00, edges01, edges10, edges11, edges20, edges21;
    cuda_scalar P00, P01, P10, P11, P20, P21;
    ...
    cuda_scalar tmp00, tmp01, tmp10, tmp11;
    cuda_scalar F00, F01, F10, F11;

    Matrix_Product_23_32(
        P00, P10, P20, P01, P11, P21,
        //
        edges00, edges01, edges10, edges11, edges20, edges21,
        //
        tmp00, tmp01, tmp10, tmp11);

    Matrix_Product_22_22(
        tmp00, tmp01, tmp10, tmp11,
        //
    ...
    // SVD
    cuda_scalar U00, U01, U10, U11;
    cuda_scalar SIG00, SIG11;
    cuda_scalar V00, V01, V10, V11;
    ...
    Matrix_Product_22_22(
        U00, U01, U10, U11,
        //
        SIG00, (cuda_scalar)0, (cuda_scalar)0, SIG11,
        //
        tmp00, tmp01, tmp10, tmp11);
    ...
    cuda_scalar PF00, PF01, PF10, PF11, PF20, PF21;
    Matrix_Product_32_22(
        P00, P01, P10, P11, P20, P21,
        //
        F00, F01, F10, F11,
        //
        PF00, PF01, PF10, PF11, PF20, PF21);

    projections_d[IDX(idO_, 0, n_projected_points)] = weight_ * PF00;
    projections_d[IDX(idO_ + 1, 0, n_projected_points)] = weight_ * PF01;
    projections_d[IDX(idO_, 1, n_projected_points)] = weight_ * PF10;
    projections_d[IDX(idO_ + 1, 1, n_projected_points)] = weight_ * PF11;
    projections_d[IDX(idO_, 2, n_projected_points)] = weight_ * PF20;
    projections_d[IDX(idO_ + 1, 2, n_projected_points)] = weight_ * PF21;

    // if(tid == 340)printf("proj %f %f %f | %d %d\n", projections_d[IDX(idO_, 0,
    // n_projected_points)] , projections_d[IDX(idO_, 1, n_projected_points)],
    // projections_d[IDX(idO_, 2, n_projected_points)], idO_, n_projected_points);

    //#ifdef DEBUG
    // if (threadIdx.x == 109)printf("proj  %f %f %f | %d \n", projections_d[IDX(idO_, 0,
    // n_projected_points)], projections_d[IDX(idO_, 1, n_projected_points)],
    // projections_d[IDX(idO_, 2, n_projected_points)], blockIdx.x); #endif
}

///////////////////////////////////////////////////////////////////////////////
__device__ void project_surface_material_and_area(
    int tid, int n_points, int n_constraints, int n_projected_points, int nb_cells,
    ShapeOpScalar *points_d, ShapeOpScalar *projections_d, cuda_scalar *f_int_nonePD_d,
    int *ConstraintType_d, int *idO_d, ShapeOpScalar *rangeMin_d, ShapeOpScalar *rangeMax_d,
    ShapeOpScalar *Scalar1_d, ShapeOpScalar *weight_d, ShapeOpScalar *E_nonePD_d, int *idI_d,
    ShapeOpScalar *vectorx_d, ShapeOpScalar *matrix22_d, ShapeOpScalar *matrix33_d,
    cuda_scalar *A_d, const float miu, const float lambda, const float kappa)
{
    ...
    if( idI0_ == 0 && idI1_ == 1 && idI2_ == 2){
        printf("rest %f %f \nrest %f %f \n", rest00_, rest01_, rest10_, rest11_);
        printf("edge %f %f \nedge %f %f  \nedge %f %f\n", edges00, edges01, edges10, edges11,
    edges20, edges21); printf("P %f %f \nP %f %f \nP %f %f  \n", P00, P01, P10, P11, P20, P21);
        printf("F %f %f \nF %f %f \n", F00, F01, F10, F11);
        printf("U %f %f \nU %f %f \n\nV %f %f \nV %f %f \n---------------------------------\n", U00,
    U01, U10, U11, V00, V01, V10, V11); printf("singular %f %f \n", SIG00, SIG11);
    }
    */
    ...
    E_nonePD_d[tid] =
        A_d[tid] * (f_tr(SIG00, miu, lambda, kappa) + f_tr(SIG11, miu, lambda, kappa)) / 2;
    ...
    */
    ...
    Matrix_Product_22_22(
        U00, U01, U10, U11,
        //
        f_prime_tr(SIG00, miu, lambda, kappa), (cuda_scalar)0., (cuda_scalar)0.,
        f_prime_tr(SIG11, miu, lambda, kappa),
        //
        tmp00, tmp01, tmp10, tmp11);
    ...
    */
    // tmp =  Piola*rest_T
    Matrix_Product_22_22(
        F00, F01, F10, F11, rest00_, rest10_, rest01_, rest11_, tmp00, tmp01, tmp10, tmp11);
    /*
  if (idI0_ == 0 && idI1_ == 1 && idI2_ == 2) {
  ...
  */
    ...
    // F = P*Piola*rest_T;
    Matrix_Product_32_22(
        P00, P01, P10, P11, P20, P21, tmp00, tmp01, tmp10, tmp11, F00, F01, F10, F11, F20, F21);

    /*
     if (idI0_ == 0 && idI1_ == 1 && idI2_ == 2) {
         printf("P*Piola*rest %f %f \nP*Piola*rest %f %f\n P*Piola*rest %f %f\n", A*F00, A*F01,
     A*F10, A*F11, A*F20, A*F21);
     }
    */
    ...
    projections_d[IDX(idO_, 0, n_projected_points)] = weight_ * PF00;
    projections_d[IDX(idO_, 0, n_projected_points)] = weight_ * PF01;
    projections_d[IDX(idO_, 1, n_projected_points)] = weight_ * PF10;
    projections_d[IDX(idO_, 1, n_projected_points)] = weight_ * PF11;
    projections_d[IDX(idO_, 2, n_projected_points)] = weight_ * PF20;
    projections_d[IDX(idO_, 2, n_projected_points)] = weight_ * PF21;
    /*
    if (idI0_ == 0 && idI1_ == 1 && idI2_ == 2) {
        printf("force \n%f %f %f\n%f %f %f\n%f %f %f\n\________________  end frame
    _________________\n\n\n", f_int_nonePD_d[IDX(idI0_, 0, n_points)], f_int_nonePD_d[IDX(idI0_, 1,
    n_points)], f_int_nonePD_d[IDX(idI0_, 2, n_points)], f_int_nonePD_d[IDX(idI1_, 0, n_points)],
    f_int_nonePD_d[IDX(idI1_, 1, n_points)], f_int_nonePD_d[IDX(idI1_, 2, n_points)],
        f_int_nonePD_d[IDX(idI2_, 0, n_points)], f_int_nonePD_d[IDX(idI2_, 1, n_points)],
    f_int_nonePD_d[IDX(idI2_, 2, n_points)]);
    }
   */
}
////////////////////////////////////////
__device__ void project_surface_material(
    int tid, int n_points, int n_constraints, int n_projected_points, int nb_cells,
    ShapeOpScalar *points_d, ShapeOpScalar *projections_d, cuda_scalar *f_int_nonePD_d,
    int *ConstraintType_d, int *idO_d, ShapeOpScalar *rangeMin_d, ShapeOpScalar *rangeMax_d,
    ShapeOpScalar *Scalar1_d, ShapeOpScalar *weight_d, ShapeOpScalar *E_nonePD_d, int *idI_d,
    ...
    cuda_scalar *A_d, const float miu, const float lambda, const float kappa)
{
    ...
    if( idI0_ == 0 && idI1_ == 1 && idI2_ == 2){
    printf("rest %f %f \nrest %f %f \n", rest00_, rest01_, rest10_, rest11_);
    printf("edge %f %f \nedge %f %f  \nedge %f %f\n", edges00, edges01, edges10, edges11, edges20,
    edges21); printf("P %f %f \nP %f %f \nP %f %f  \n", P00, P01, P10, P11, P20, P21); printf("F %f
    %f \nF %f %f \n", F00, F01, F10, F11); printf("U %f %f \nU %f %f \n\nV %f %f \nV %f %f
    ...
    */
    ...
    cuda_scalar A = -0.5 * A_d[tid];
    // none linear business
    E_nonePD_d[blockIdx.x * n_constraints + tid] =
        A_d[tid] * (f_tr(SIG00, miu, lambda, kappa) + f_tr(SIG11, miu, lambda, kappa)) / 2;
    // printf("e_non_pd %f %f\n", E_nonePD_d[tid], A_d[tid]);
    /*
    if (idI0_ == 0 && idI1_ == 1 && idI2_ == 2) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
__device__ void nearest_neighbor_linear_d(
    double *points, double *colid_points, double *colid_normals, double *nearest_points,
    ...
    const float threshold_rep, const float threshold_nonRep)
{
    ...
    if (argmin > -1) {
        if (Norm_3(colid_normals[argmin], colid_normals[argmin + 1], colid_normals[argmin + 2])
            >= 2.0) {
            threshold = threshold_rep;

            // 0.5 is to decode the info of repulsion
            nearest_points[IDX(id, 0, n_points)] =
                colid_points[argmin] + threshold * 0.5 * colid_normals[argmin] - center[0];
            nearest_points[IDX(id, 1, n_points)] =
                colid_points[argmin + 1] + threshold * 0.5 * colid_normals[argmin + 1] - center[1];
            nearest_points[IDX(id, 2, n_points)] =
                colid_points[argmin + 2] + threshold * 0.5 * colid_normals[argmin + 2] - center[2];
            ...
            if (min <= threshold) {
                nearest_points[IDX(id, 0, n_points)] =
                    colid_points[argmin] + threshold * colid_normals[argmin] - center[0];
                nearest_points[IDX(id, 1, n_points)] =
                    colid_points[argmin + 1] + threshold * colid_normals[argmin + 1] - center[1];
                nearest_points[IDX(id, 2, n_points)] =
                    colid_points[argmin + 2] + threshold * colid_normals[argmin + 2] - center[2];
...
//////////////////////////////////////////////////////////////////////////////
__device__ void project_collision_d(
    int n_points, int nb_cells, double *points, double *nearest_points, double *nearest_normals,
    cuda_scalar *f_int_nonePD_d, double *E_nonePD, const int id, const int n_constraints,
    const float weight_col_rep, const float threshold_rep, const float weight_col_nonRep,
    const float threshold_nonRep, const float beta_morse)
{
    float dif[3];
    ...
    if (DOT_3(
            dif[0], dif[1], dif[2], nearest_normals[IDX(id, 0, n_points)],
            nearest_normals[IDX(id, 1, n_points)], nearest_normals[IDX(id, 2, n_points)])
        < 0.)
    {
        if (Norm_3(
                nearest_normals[IDX(id, 0, n_points)], nearest_normals[IDX(id, 1, n_points)],
                nearest_normals[IDX(id, 2, n_points)])
            >= 2.0)
        {
            // 0.5 is to decode the info of repulsion

            /* Morse potential
            f_int_nonePD_d[IDX(id, 0, n_points)]
            -= 2.0f*weight_col_rep*beta_morse*(expf(2.0f*beta_morse*dif[0]) -
            expf(beta_morse*dif[0])); f_int_nonePD_d[IDX(id, 1, n_points)]
            -= 2.0f*weight_col_rep*beta_morse*(expf(2.0f*beta_morse*dif[1]) -
            expf(beta_morse*dif[1])); f_int_nonePD_d[IDX(id, 2, n_points)]
            -= 2.0f*weight_col_rep*beta_morse*(expf(2.0f*beta_morse*dif[2]) -
            expf(beta_morse*dif[2]));

            E_nonePD[blockIdx.x*n_constraints + threadIdx.x] +=
            weight_col_rep*(expf(2.0f*beta_morse*dif[0]) - 2.0f*expf(beta_morse*dif[0])) +
                                                                weight_col_rep*(expf(2.0f*beta_morse*dif[1])
            - 2.0f*expf(beta_morse*dif[1])) + weight_col_rep*(expf(2.0f*beta_morse*dif[2])
            - 2.0f*expf(beta_morse*dif[2]));
            //*/

            /* Custom Potential
            // "beta_morse" 40.0 is a good value
            dif[0] = points[IDX(id, 0, n_points)] - (nearest_points[IDX(id, 0, n_points)] +
            (beta_morse - threshold_rep)*0.5*nearest_normals[IDX(id, 0, n_points)]); dif[1] =
            points[IDX(id, 1, n_points)] - (nearest_points[IDX(id, 1, n_points)] + (beta_morse -
            threshold_rep)*0.5*nearest_normals[IDX(id, 1, n_points)]); dif[2] = points[IDX(id, 2,
            n_points)] - (nearest_points[IDX(id, 2, n_points)] + (beta_morse -
            threshold_rep)*0.5*nearest_normals[IDX(id, 2, n_points)]);
            ...
            E_nonePD[blockIdx.x*n_constraints + threadIdx.x] += weight_col_rep*(dif[0] * dif[0] +
            dif[1] * dif[1] + dif[2] * dif[2]) / 2.0f;
            //*/
            ...
            E_nonePD[blockIdx.x * n_constraints + threadIdx.x] +=
                weight_col_rep * (dif[0] * dif[0] + dif[1] * dif[1] + dif[2] * dif[2]) / 2.0f;
            //*/
        } else {
            ...
            E_nonePD[blockIdx.x * n_constraints + threadIdx.x] +=
                weight_col_nonRep * (dif[0] * dif[0] + dif[1] * dif[1] + dif[2] * dif[2]) / 2.0f;
        }
    }
}
