...
/**
 * This function returns a pointer to the IsoThermalBulkDynamics base class. Depending on the collision_type parameter
 * it assign to the returned pointer a pointer to a different type of dynamics, either BGK or TRT. The TRT class has a
 * variant defined using a linearized equilibrium, its name is Ma1TRTdynamics. This functions set the omega- parameter
 * of the TRT class starting from the magic parameter Lambda. The omega- is automaticcally recomputed when setting the
 * magic parameter.
 * @param basicString
 * @param tau_plus
 * @param magic
...
int main(int argc, char* argv[]) {
    ...
    if(tau_plus > 2.0){
        pcout << "ERROR! The computed value of relaxation parameter tau_plus+= " << tau_plus <<" is too high.\n";
        abort();
    }
