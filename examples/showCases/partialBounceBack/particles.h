...
public:
    ...
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D *> blocks)
    {
        ...
        ScalarField3D<plint> &numParticlefield = *dynamic_cast<ScalarField3D<plint> *>(
            blocks[1]);  // ScalarField that will contain the qty of tPA in the domain
...
void setupCouplings(
    MultiBlockLattice3D<T, DESCRIPTOR> *lattice, MultiParticleField3D<ParticleFieldT> *particles,
    MultiScalarField3D<T> *sF)
{
    ...
    integrateProcessingFunctional(
        new PBBFluidToParticleCoupling3D<T, DESCRIPTOR>(1.), lattice->getBoundingBox(),
        particleFluidArg, 0);

    // Calls the function advance() on each particle which, for Verlet particles,
    // updates the position according to v(t+0.5)
    integrateProcessingFunctional(
        new AdvanceParticlesFunctional3D<T, DESCRIPTOR>(-1.0), lattice->getBoundingBox(),
        particleArg, 0);
...
{
    pcout << "# total particles = " << countParticles(*particles, particles->getBoundingBox())
          << ", writing number of particles per cell VTK...\n";
    plint startPartIter = simParam.startParticleTime / simParam.dt;

    writeParticleVtk(
        *particles,
        std::string(
            simParam.outDir + "vtk/particles"
            + std::to_string(
                (simParam.overwriteVtk
                     ? ((iT >= startPartIter && iT <= startPartIter + simParam.outIter + 1) ? 0 : 1)
                     : iT))
            + ".vtk")
            .c_str(),
        simParam.dx);
...
// data processor to create a given number of particles randomly in a circular area, and setting
// their age
...
public:
    InjectParticlesAtInletSlice(plint iter_, plint numParticles_, DomainFunctional functional_) :
        iter(iter_), numParticles(numParticles_), functional(functional_)
    { }
    InjectParticlesAtInletSlice(
        InjectParticlesAtInletSlice<T, Descriptor, DomainFunctional> const &rhs) :
        iter(rhs.iter), numParticles(rhs.numParticles), functional(rhs.functional)
    { }
    InjectParticlesAtInletSlice<T, Descriptor, DomainFunctional> &operator=(
        InjectParticlesAtInletSlice<T, Descriptor, DomainFunctional> const &rhs)
    {
        InjectParticlesAtInletSlice<T, Descriptor, DomainFunctional>(rhs).swap(*this);
        return this;
    }
    ...
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D *> blocks)
    {
        ...
        plint numVoxelsBeforeClot = floor(
            pow(radius, 2) * 3.1416
            * (floor(simParam.clotBeginZ * simParam.nz)
               + 1));  // number of voxels inside the artery, before the clot. Useful to compute how
                       // many particles to inject in a given subdomain (parallelization).
        // plint numVoxelsHere = floor(pow(radius,
        // 2)*3.1416*(std::min(abs(double(domain.z1+offset.z)),abs(floor(simParam.clotBeginZ*simParam.nz)-(domain.z0+offset.z)))));
        plint numVoxelsHere = floor(pow(radius, 2) * 3.1416 * double(domain.z1 + offset.z));

        plint partsToInjectInThisSubdomain =
            ceil(numParticles * (double)numVoxelsHere / (double)numVoxelsBeforeClot);
        ...
        for (size_t i = 0; i < partsToInjectInThisSubdomain; ++i) {
            ...
            do {
                ...
                cy = uni_y(rng);
                cz = uni_z(rng);
            } while (!functional(Array<T, 3>(
                cx + offset.x, cy + offset.y,
                cz + offset.z)));  // do this until we are in the domain functional

            particleTemplate = new PointParticle3D<T, DESCRIPTOR>(
                simParam.antiFnQtyPerCell, Array<T, 3>(cx + offset.x, cy + offset.y, cz + offset.z),
                Array<T, 3>(0., 0., 0.));
...
void injectParticles(plint iter, plint numPart, MultiParticleField3D<ParticleFieldT> *particles)
{
    ...
    Box3D injectionDomain(
        2, simParam.nx - 3, 2, simParam.ny - 3, 0, floor(simParam.clotBeginZ * (simParam.nz)) + 1);
    ...
    applyProcessingFunctional(
        new InjectParticlesAtInletSlice<T, DESCRIPTOR, Pipe<T>>(iter, numPart, poiseuillePipe),
