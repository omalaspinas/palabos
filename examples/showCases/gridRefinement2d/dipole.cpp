...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        pcout << "where N is the resolution, and computePoisson if the poisson equation is solved."
              << std::endl;
        exit(1);
    }
    ...
    for (plint iLevel = 0; iLevel < lattice.getNumLevels(); ++iLevel) {
        ...
        if (computePoisson) {
            ...
            plb_ifstream poisson(
                createFileName("GS", lattice.getComponent(iLevel).getNx(), 6).c_str());
            poisson >> press;
