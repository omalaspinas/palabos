...
void simulationSetup(MultiBlockLattice3D<T, DESCRIPTOR> &lattice, const Param<T> &param)
{
...
void writeGif(MultiBlockLattice3D<T, DESCRIPTOR> &lattice, Param<T> &param, plint iter)
{
    ...
    imageWriter.writeScaledGif(
        createFileName("vorticity", iter, 6),
        *computeNorm(*computeVorticity(*computeVelocity(lattice)), slice), imSize, imSize);
}
/// Output data into a VTK file.
void writeVTK(MultiBlockLattice3D<T, DESCRIPTOR> &lattice, Param<T> &param, plint iter)
{
    T dx = param.dx;
    T dt = param.dt;
    ...
    vtkOut.writeData<3, float>(
        *computeVorticity(*computeVelocity(lattice)), "vorticity", (T)1 / dt);
}
...
int main(int argc, char *argv[])
{
    ...
    MultiBlockLattice3D<T, DESCRIPTOR> lattice(param.nx, param.ny, param.nz, dyn);
