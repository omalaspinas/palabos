...
public:
    ...
    void operator()(plint, plint iY, plint iZ, Array<T, 3> &u) const
    {
    ...
    for (plint iT = 0; iT * parameters.getDeltaT() < maxT; ++iT) {
        if (iT % parameters.nStep(imSave) == 0) {
            if (iT % parameters.nStep(imSave) == 0 && iT * parameters.getDeltaT() > 1.) {
                pcout << "Writing VTK file at time " << iT * parameters.getDeltaT() << endl;
                writeVTK(lattice, parameters, iT);
            } else {
                pcout << "Current time of simulation is " << iT * parameters.getDeltaT()
